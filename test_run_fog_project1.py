...
def remainItems(item,nodeIdx,weights):
    remUsers=[]
...
def runner_code(noOfFogNodes,capacity,alpha,noOfEndUsers,input_data,clock_cycles,clock_speed_user,clock_speed_fog,upload_rate,download_rate):
    ...
    for idx,value in enumerate(answer):
        y=  remainItems(value[0],idx,weights)
        if y:
    ...
    print(solveMultiKnapsack(remNodes,newCapacity,len(remUsersWeights),remUsersWeights))
