...
def findMaximumTime(answer, t):
    ...
    return maxTime
def findMaxTimeMigrated(answer,t):
    maxTime = 0
    for i,a in enumerate(answer):
          if a=='1':
...
def remainItems(item,nodeIdx,time_cpu_user):
    ...
    for i in range(len(item)):
        if item[i]=='0':
            ...
            time_cpu.append(time_cpu_user[nodeIdx][i])
            
    return (remUsers,time_cpu)
def main(capacity,noOfFogNodes,noOfEndUsers,answer,time_cpu_user_new,weight):
    global weights
    weights=weight
    standTime=0
    time_fed_cpu=time_cpu_user_new.copy()
    answer=[]
    # noOfFogNodes=3
    # noOfEndUsers=[]
    # standTime=0
    ...
    # print(answer)
    standTime+=findMaximumTime(answer,time_fed_cpu)
    ...
    for idx,value in enumerate(answer):
        ...
        y,time_for_ind_node=  remainItems(value[0],idx,time_fed_cpu)
        time_migrate+=time_for_ind_node
        if value[1]==0:
            return (-1)
