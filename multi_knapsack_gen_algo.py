...
def genRandIntegerString(n,noOfKnapsacks):
    number=''.join([str(random.choice([0,(random.randint(1, noOfKnapsacks))])) for i in range(n)])
    return number
...
def solveMultiKnapsack(initNoOfKnapsack,init_capacity,noOfItems,weights):
    ...
    population=[genRandIntegerString(n,noOfKnapsacks) for i in range(setInitpop)]
